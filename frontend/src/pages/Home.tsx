...
import { ArrowRight, Users, Settings, Fuel, Calendar } from "lucide-react";
...
export default function Home() {
  ...
  const featuredVehicles = vehicles.filter((v: { available?: boolean }) => v.available).slice(0, 3);
        ...
        <div className="container mx-auto px-6 lg:px-12">
          ...
          {featuredVehicles.length > 0 ? (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {featuredVehicles.map((vehicle: { id: number; make: string; model: string; year: number; weeklyRate: number; available?: boolean; imageUrl?: string }, idx: number) => (
                <motion.div
                  key={vehicle.id}
                  initial={{ opacity: 0, y: 20 }}
                  whileInView={{ opacity: 1, y: 0 }}
                  ...
                  <Card className="overflow-hidden hover:shadow-xl transition-shadow duration-300">
                    <CardContent className="p-0">
                      ...
                      <div className="relative">
                        <img
                          src={vehicle.imageUrl || `https://images.unsplash.com/photo-1494976388531-d1058494cdd8?w=600&q=80`}
